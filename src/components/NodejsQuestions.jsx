...
import { ChevronDown, ServerCog, ArrowLeft, Search, Eye, EyeOff, CheckCircle2, Circle, FilterX, Copy, Check } from 'lucide-react';
                                        ...
                                        <div className={`pr-4 sm:pr-8 transition-all duration-500 ${isBlurred ? 'blur-md select-none opacity-50' : 'blur-none opacity-100'}`}>
                                            ...
                                            {q.answer.split(/(?<=\.|\?|\!)(?=\s+[A-Z])/).map((sentence, i) => (
                                                <div key={i} className={`flex items-start ${i !== 0 ? "mt-4" : ""}`}>
                                                    <div className="flex-shrink-0 w-1.5 h-1.5 rounded-full mt-2.5 mr-3 bg-emerald-400 shadow-[0_0_8px_rgba(16,185,129,0.8)]"></div>
